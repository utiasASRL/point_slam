...
void PointMapSLAM::processCloud(const sensor_msgs::PointCloud2::ConstPtr& msg, bool filtering, bool update_map)
{
	...
	// Get angle for each lidar point
	vector<float> lidar_angles;
	get_lidar_angles(polar_pts, lidar_angles, params.lidar_n_lines);

	// ROS_WARN_STREAM(" ---> Found " << lidar_angles.size() << " angles");
	// for (auto a: lidar_angles)
	// 	ROS_WARN_STREAM(" ------> " << a);
...
int main(int argc, char **argv)
{
	...
	{
		slam_params.log_path = "/home/" + user_name + "catkin_ws/src/point_slam_2/src/test_maps/results/";
